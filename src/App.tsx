...
export const CHEMICALS: Record<string, Chemical> = {
    ...
    'ZINC': { id: 'ZINC', name: { VN: 'Kẽm', EN: 'Zinc' }, formula: 'Zn', color: '#a1a1aa', type: 'solid', meshStyle: 'rock', ph: 7.0, description: { VN: 'Kim loại chuyển tiếp.', EN: 'Transition metal.' }, category: 'basic', atomicNumber: 30 },
...
class GeminiService {
    ...
    async callGeminiAPI(userMessage: string): Promise<string> {
        // SECURITY: Encode user-provided API key to prevent HTTP parameter injection
        const safeApiKey = encodeURIComponent(this.apiKey || '');
        const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${safeApiKey}`;
