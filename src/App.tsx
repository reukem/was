...
const LabScene: React.FC<{
    ...
    useEffect(() => { onMoveRef.current = onMove; onPourRef.current = onPour; onDropRef.current = onDrop; }, [onMove, onPour, onDrop]);
                ...
                if (group) {
                    ...
                    meshesRef.current.forEach((otherGroup, otherId) => {
                        if (id !== otherId && !poured && otherId !== 'ANALYZER_MACHINE') {
                            ...
                            if (dist < 1.4) {
                                // Priority: Drop if solid, Pour if liquid
                                const srcContainer = containers.find(c => c.id === id);
                                const chem = CHEMICALS[srcContainer?.contents?.chemicalId || ''];
                                if (chem?.type === 'solid' || chem?.meshStyle === 'rock') {
                                     onDropRef.current?.(id, otherId);
                                } else {
